...
bool spinXDegrees(int direction, float degree)
{
	...
	float distanceToTravel = distanceForDegree(degree) ;		// D√©finition de la distance d'un degr√©e.
...
int spinXDegreesByHoles(int direction, float degree)
{
	...
	//QuantitÈ de trous d'encodeur que les roues doivent tourner pour faire une rotation de X degree
	float holesToTravel = holesForDistance(distanceForDegree(degree));		// DÈfinition de la distance d'un degrÈe.

	// DÈfinition des valeurs des distances parcourues Temporaire.
	int holesLeft = 0;
	int holesRight = 0;
	...
	// On rÈinitialise les moteurs a 0.
	MOTOR_SetSpeed (MOTOR_LEFT,0);
	MOTOR_SetSpeed (MOTOR_RIGHT,0);
